...
const Work = ({ index, title, description, skills, demoLink, githubLink, preview }) => {
  ...
  useEffect(() => {
    tl
      .to(titleRef.current, 0.45, { opacity: 1, y: 0 })
  ...
  return (
    <section className="work">
      <div className="work__info">
        <h1 className="work__title h1 slide-down" ref={titleRef}>{title}</h1>
        <p className="work__subtitle slide-down"
           dangerouslySetInnerHTML={{ __html: description }} ref={subtitleRef}/>
        <ul className="work__techs slide-down" ref={skillsRef}>
          {skills.split(` `).map((skill) => <li key={skill}>{skill} </li>)}
        </ul>
        <div className="work__buttons only-for-desktop slide-down" ref={buttonsRef}>
